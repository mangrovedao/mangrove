...
async function nextOfferId(base, quote, mgv) {
  const [, local] = await mgv.configInfo(base, quote);
...
async function marketOrder(
  ...
  return [takerGot, takerGave, bounty];
}
...
async function deployMangrove() {
  ...
  await mgv.deployed();
  const receipt = await mgv.deployTransaction.wait();
  console.log(
...
function listenOfferLogic(logic) {
  const filter_fail = logic.filters.GetFail();
  logic.on(filter_fail, (outTkn, inTkn, offerId) => {
    console.log(outTkn, inTkn, offerId);
    stopListeners([logic]);
  });
  const filter_putfail = logic.filters.PutFail();
  logic.on(filter_putfail, (outTkn, inTkn, offerId) => {
    console.log(outTkn, inTkn, offerId);
    stopListeners([logic]);
  });
}
...
async function logOrderBook(
  ...
  const out_dec = await outbound_tkn.decimals();
  const in_dec = await inbound_tkn.decimals();
  const out_sym = await outbound_tkn.symbol();
  const in_sym = await inbound_tkn.symbol();
  console.log(chalk.black.bgBlue(`====(${out_sym},${in_sym})====`));
  let cpt = 0;
  offerIds.forEach((offerId, i) => {
    if (offerId != 0) {
      ...
      console.log(
        ...
        ":",
        formatToken(offer.gives, out_dec),
        formatToken(offer.wants, in_dec),
        chalk.gray(offerDetails[i].maker.toString())
      );
    }
  });
  if (cpt == 0) {
